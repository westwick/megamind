...
class ConversationHandler {
      ...
      for (let i = 0; i < event.message.spans.length - 1; i++) {
        ...
        if (
          ...
          if (match) {
            ...
            console.log(`[conversationHandler] ${sender} telepaths: ${fullMessage}`);
  ...
  checkForBroadcast(event) {
    if (event.message && event.message.spans && event.message.spans.length > 0) {
      const isBrightYellow = event.message.spans.some(span => 
        span.color &&
      ...
      if (isBrightYellow) {
        const fullMessage = this.getFullMessageText(event.message);
        ...
        if (match) {
          ...
          console.log(`[conversationHandler] Broadcast from ${username}: ${message}`);
        }
      }
    }
  }
  ...
  checkForGossipsAndAuctions(event) {
    ...
    if (event.message && event.message.spans && event.message.spans.length > 0) {
      const relevantSpans = event.message.spans.filter(span => 
        (span.color && span.color.name === "lightGray" && (span.text.includes("gossips:") || span.text.includes("auctions:"))) ||
        (span.color && span.color.name === "magenta")
      );

      if (relevantSpans.length >= 2) {
        const cleanMessage = event.line.split(']:').pop().trim();
        const [senderPart, messagePart] = cleanMessage.split(/\s+(gossips|auctions):\s+/);
        ...
        if (senderPart && messagePart) {
          ...
          console.log(`[conversationHandler] ${type.charAt(0).toUpperCase() + type.slice(1)} from ${username}: ${message}`);
        }
      }
    }
  }
  ...
  checkForGangpath(event) {
    console.log("[conversationHandler] checkForGangpath", JSON.stringify(event));
    if (event.message && event.message.spans && event.message.spans.length > 0) {
      const gangpathSpans = event.message.spans.filter(span => 
        (span.color && span.color.name === "green" && !span.color.bright && span.text.includes("gangpaths:")) ||
      ...
      if (gangpathSpans.length >= 2) {
        ...
        if (senderPart && messagePart) {
          ...
          console.log(`[conversationHandler] Gangpath from ${username}: ${message}`);
        }
      }
    }
  }
  ...
  checkForPaging(event) {
    if (event && event.messages && event.messages.length >= 2) {
      ...
      if (this.hasYellowAsterisks(firstMessage)) {
        // Check for the paging pattern in the second message
        const pagingPattern = /^(\u0007)?(\w+) is paging you from (\w+): (.+)$/;
        const secondMessageText = this.getFullMessageText(secondMessage);
        ...
        if (match) {
          ...
          console.log(`[conversationHandler] ${username} paged from ${module}: ${message}`);
        }
      }
    }
  }

  hasYellowAsterisks(message) {
    if (message && message.spans && message.spans.length > 0) {
  ...
  getFullMessageText(message) {
    if (message && message.spans) {
      return message.spans.map(span => span.text).join('');
    }
    return '';
  }
}
