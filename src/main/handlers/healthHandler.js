...
class HealthHandler extends Handler {
  ...
  handleHealth(event) {
    ...
    if (currentHealth !== this.gameState.player.health || currentMana !== this.gameState.player.mana) {
  ...
  async handleHealthUpdate(health, mana) {
    ...
    const hangIfBelowValue = Math.floor(maxHealth * (hangIfBelowPercentage / 100));
    ...
    if (health && health < 25 && health > 1) {
      ...
      this.currentStrategy = new HealingStrategy(this.gameState, this.commandManager, this.eventBus);
