<template>
  <div class="dynamic-settings">
    <div v-for="(value, key) in modelValue" :key="key" class="setting-group">
      <h3 class="group-title">{{ formatLabel(key) }}</h3>
      <component
...
const ObjectInput = {
  ...
  template: `
    <CollapsibleWrapper
      ...
      @toggle="isExpanded = !isExpanded"
    >
      <div v-if="isExpanded" class="object-properties">
      ...
      <div v-else class="preview">
        {{{ Object.keys(modelValue).length }} properties}
      </div>
    </CollapsibleWrapper>
  `,
  methods: {
